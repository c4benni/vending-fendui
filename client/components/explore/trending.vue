...
export default {
  ...
  computed: {
    ...
    allItems() {
      return this.$store.state.explore.trending || []
    },
  ...
  methods: {
    ...
    async fetchGroup() {
      ...
      const saved = this.$store.state.fetchTimeStamps.trending || 0
    ...
    return appSection(
      {
        attrs: {
          ...
          description: this.showDescription
            ? 'Discover the items breaking our charts!'
            : undefined,
        },
        staticClass: 'e-tg',
      },
        ...
        btn(
          {
            props: {
              ...
              to: getViewAllTo.call(this, {
                variable: `$l$:10,$n$:recipe,$t$:trending,$r$:id,$f$:f_bee`,
                title: '$t$ $n$s',
                showingFor: '$n$',
                fetched: ':c',
                get: '$f${',
...
<style>
.can-hover .e-tg:hover .view-all:not(.Active):not(:hover):not(:focus)::before {
  opacity: 0.1;
}

.scroll-wrap[data-e-tg] {
